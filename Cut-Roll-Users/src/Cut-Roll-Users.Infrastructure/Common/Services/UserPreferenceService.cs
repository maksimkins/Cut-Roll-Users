    ...
    private MovieDataForEmbeddingDto? ConvertMovieToEmbeddingData(Movie movie)
    {
        try
        {
            return new MovieDataForEmbeddingDto
            {
                ...
                Keywords = movie.Keywords?.Select(mk => mk.Keyword.Name).ToList() ?? new List<string>(),
                Cast = movie.Cast?.Select(c => c.Person.Name).ToList() ?? new List<string>(),
                Crew = movie.Crew?.Select(c => c.Person.Name).ToList() ?? new List<string>(),
                ProductionCompanies = movie.ProductionCompanies?.Select(pc => pc.Company.Name).ToList() ?? new List<string>(),
                ProductionCountries = movie.ProductionCountries?.Select(pc => pc.Country.Name).ToList() ?? new List<string>(),
                SpokenLanguages = movie.SpokenLanguages?.Select(sl => sl.Language.EnglishName).ToList() ?? new List<string>(),
